...
describe("temple manual survey market", () => {
  ...
  it("prices A/S from crystal E[p] and Trash from dump (~60); blank buy stays NaN", () => {
    ...
    // Crystal 1p1s land in SS (crystal-presence force); A cascades toward dump
    expect(sales.uncorrupted.SS).toBeGreaterThan(900);
    expect(sales.uncorrupted.SS).toBeLessThan(2100);
    expect(sales.uncorrupted.S).toBeLessThanOrEqual(sales.uncorrupted.SS);
    ...
    expect(sales.uncorruptedSource.SS).toBe("manual-survey");
  });
